...
export class CheckoutPaymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    loadStripe("pk_test_51MZOnQE2nCh4pQvl7tPaCvsrQ4WmbjJMfY9ZqZhXdicLrBHcnVPRTbLIAAMMhcgeKpcJrVyCzhXU8B96htccHeqB007URW6aHm")
      .then(stripe => {
        this.stripe = stripe
  ...
  private async confirmPaymentWithStripe(basket: Basket | null) {
    ...
    const result = this.stripe?.confirmCardPayment(basket.clientSecret!, {
      payment_method: {
        ...
        billing_details: {
          name: this.checkoutForm?.get('paymentForm')?.get('nameOnnCard')?.value
        }
      }
    })
    if (!result) throw new Error('Problem ateempting payment with stripe')
    return result
  }
